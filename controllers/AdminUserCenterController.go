...
type AdminUserCenterController struct {
	BaseController
}

func (c *AdminUserCenterController) Prepare() {
	//先执行
	c.BaseController.Prepare()
	//如果一个Controller的所有Action都需要登录验证，则将验证放到Prepare
	c.checkLogin()
}
func (c *AdminUserCenterController) Profile() {
	Id := c.curUser.Id
	m, err := models.AdminBackendUserOne(Id)
	if m == nil || err != nil {
...
func (c *AdminUserCenterController) BasicInfoSave() {
	Id := c.curUser.Id
	oM, err := models.AdminBackendUserOne(Id)
	if oM == nil || err != nil {
...
func (c *AdminUserCenterController) PasswordSave() {
	Id := c.curUser.Id
	oM, err := models.AdminBackendUserOne(Id)
	if oM == nil || err != nil {
		c.pageError("数据无效，请刷新后重试")
...
func (c *AdminUserCenterController) UploadImage() {
	//这里type没有用，只是为了演示传值
